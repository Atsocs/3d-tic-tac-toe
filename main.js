{ // Constants
var Q = 4;
var hard_coded_velha = [1, 1, 2, 0, 1, 1, 1, 2, 2, 2, 2, 1, 1, 2, 1, 2, 2, 2, 1, 2, 2, 2, 2, 1, 2, 1, 2, 1, 1, 2, 2, 1, 1, 2, 1, 1, 1, 1, 2, 1, 1, 2, 1, 2, 2, 1, 1, 2, 1, 2, 1, 1, 2, 1, 2, 2, 1, 2, 2, 1, 1, 2, 2, 1]
...
var ocup = 0;
var mmCounter = 0; mmTotalCounter = 0;
var party = false; var timer;
var scoreText = "";
...
var Mx = 0, My = 0, Mz = 0;
var Mx = 0, My = 0, Mz = 0;
var c = document.createElement('canvas'); var container = document.getElementById("cont");
container.appendChild(c); container.insertBefore(c, container.childNodes[8]);
var scoreE = document.getElementById("score"); var processorE = document.getElementById("processor");
var ctx = c.getContext("2d");
var Px = 0, Py = 0;
var eqA = "Azul", eqL = "Laranja", eqV = "Verde";
var trofeus = {};
}
//start();
showOptions();
...
function start(){
	...
	var EqA = document.getElementById("eqA"); eqA = EqA.value;
	var EqL = document.getElementById("eqL"); eqL = EqL.value;
	var EqV = document.getElementById("eqV"); eqV = EqV.value;
	...
	if (players == 1){players = 2; AIplaying = true; eqL = "Unbeatable"}
...
function put(){
	if(running){
		...
		if (board[Marr] == 0){
			...
			entries.push(Marr);
			empty_spaces--;
			if (!AIplaying){turn = (turn + 1) % players; beep(); //console.log(empty_spaces);
}
			if (AIplaying){
				beep();
				...
				setTimeout(function(){
					...
					for (var i = 0; i < dif.length; i++){
					 ...
					 empty_spaces--; entries.push(AIM); beep(); console.log("processing_steps=" + mmCounter + " empty_spaces=" + empty_spaces); mmTotalCounter += mmCounter;
					processorE.innerHTML = mmCounter; mmCounter = 0;
				}, 200);
			}
			load(board);
		}
			...
			if (VARR > 0){
				...
				if (Rx < Q && Ry < Q && Rz < Q && Rx >= 0 && Ry >= 0 && Rz >= 0){
					...
					if(testLine(n, RARR, board)){
						...
						timer = setTimeout(function(){displayConfirm(); }, 7000);
					}
				}
				if (empty_spaces <= 0){
					running = false;
					...
					timer = setTimeout(function(){playagain();}, 3000);
				}
			}
		}
	}
...
function testTrophy(){
	...
	for (var i = 0; i < score.length; i++){
		if (score[i] >= games){
			if (i == 0){msg = "O time " + eqA + " ganhou um Troféu!"; addTrophy(eqA);}
			else if (i == 1 && AIplaying) {msg = "Unbeatable continua imbatível..."; addTrophy("Unbeatable");}
			else if (i == 1 && !AIplaying) {msg = "O time " + eqL + " ganhou um Troféu!"; addTrophy(eqL);}
			else if (i == 2 && !AIplaying) {msg = "O time " + eqV + " ganhou um Troféu!"; addTrophy(eqV);}
			setTimeout(function(){settings(true)}, 200);
		}
	}
	if(msg == "???") return false;
	if(msg != "???") return msg;
...
function minimax (state = board, isMaximizingPlayer = true, depth = 0){
	mmCounter++;
	if (terminalState(state)) return terminalState(state); //It will be executed if state is terminal
...
function load(state = board){
	var azul = eqA + " " + score[0], laranja = score[1] + " " + eqL, verde = score[2] + " " + eqV;
	...
	var res = azul + " | " + laranja; if(players == 3){res += " | " + verde};
...
if (party === true && running === false && started === true){
		party = false; load(winBoard);
	} else if (party === false && running === false && started == true){
		party = true; load(winBoardS)
	};
}, 250);
